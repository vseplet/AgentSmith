...
async function moltbookFetch(
  ...
  console.log(`[Moltbook] Request: ${options.method || "GET"} ${endpoint}`);
  ...
  console.log(
    `[Moltbook] Response: ${response.status}`,
...
function solveMathChallenge(challenge: string): string | null {
  ...
  console.log(`[Moltbook] Decoded challenge: ${decoded}`);
  ...
  console.log(`[Moltbook] After number replacement: ${text}`);

  const numbers = text.match(/\d+/g)?.map(Number);
  console.log(`[Moltbook] Extracted numbers: ${numbers}`);
  ...
  console.log(`[Moltbook] Calculated answer: ${result}`);
    ...
    switch (action) {
      ...
      case "post": {
        ...
        if (result.verification_required && result.verification) {
          ...
          if (answer) {
            console.log(
              `[Moltbook] Auto-verifying post with answer: ${answer}`,
            );
      ...
      case "comment": {
        ...
        if (result.verification_required && result.verification) {
          ...
          const answer = solveMathChallenge(verification.challenge);
          if (answer) {
            console.log(`[Moltbook] Auto-verifying with answer: ${answer}`);
            const verifyResult = await moltbookFetch("/verify", {
              method: "POST",
              body: JSON.stringify({
                verification_code: verification.code,
